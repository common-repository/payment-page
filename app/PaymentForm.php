...
class PaymentForm {
  ...
  public function get_from_payment_form_id($post_id) {
    try {
      return $this->get_from_settings( PaymentPage_PostType_Form_FieldMap::get_form_settings_mapped_as_key_value($post_id), $post_id );
    } catch ( \Exception $e ) {
      return '';
    }
  }
	...
	public function get_from_elementor_settings( $settings, $post_id ) {
		try {
			return $this->get_from_settings( $settings, $post_id );
		} catch ( \Exception $e ) {
			return '';
		}
	}

	public function get_from_settings( $settings, $post_id ) {
		$uniqid            = uniqid();
		...
		foreach ( $payment_methods_map as $_gateway => $payment_methods ) {
			foreach ( $payment_methods as $payment_method_key => $payment_method ) {
				if ( ! isset( $settings[ 'payment_method_' . $_gateway . '_' . $payment_method ] )
				     || $settings[ 'payment_method_' . $_gateway . '_' . $payment_method ] !== 'yes' ) {
    ...
    $submit_trigger_label = null;

    if(isset($settings['submit_trigger_label'])) {
      $submit_trigger_label = $settings['submit_trigger_label'];
    } else if(isset($settings['sorted_text_button'])) {
    ...
    $pricing_plan_option_label = null;

    if(isset($settings['pricing_selector_label']['token_order'])) {
      $pricing_plan_option_label = $settings['pricing_selector_label'];
    } else if(isset($settings['pricing_selector_label']['sorted'])) {
      ...
      'is_live_preview'             => ( !empty($settings[ 'is_live_preview' ]) ? $settings[ 'is_live_preview' ] : 0),
			'currency_symbol'             => ( isset( $settings['currency_symbol'] ) && ( $settings['currency_symbol'] === 'no' || empty( $settings['currency_symbol'] ) ) ? 0 : 1 ),
			'subscription_selector'       => ( isset( $settings['subscription_selector'] ) && $settings['subscription_selector'] === 'yes' ? 1 : 0 ),
			'currency_selector'           => ( isset( $settings['currency_selector'] ) && $settings['currency_selector'] === 'yes' ? 1 : 0 ),
			'post_id'                     => $post_id,
			'domain_name'                 => payment_page_domain_name(),
			'pricing_plans'               => [],
			'field_map'                   => $settings['form_fields_map'],
			'submit_actions'              => $settings['submit_actions'],
			'submit_trigger_label'        => $submit_trigger_label,
			'pricing_plan_option_label'   => $pricing_plan_option_label,
			'lang'                        => [
			...
			'payment_gateways'            => (
			                                 empty( $payment_methods_map['stripe'] ) && empty( $payment_methods_map['paypal'] )
				                                 ? [
				                                 'skeleton' => [
					                                 'warning'         => (
					                                 current_user_can( PAYMENT_PAGE_ADMIN_CAP )
						                                 ? '<div data-payment-page-notification="danger" style="text-align:center !important;">' .
						                                   __( "No payment gateway connected.", "payment-page" ) . '<br/>' .
						                                   '<a href="' . esc_url( admin_url( 'admin.php?page=' . PAYMENT_PAGE_MENU_SLUG ) ) . '#payment-gateways" target="_blank">' .
						                                   __( "Connect a payment gateway >", "payment-page" ) .
						                                   '</a>' .
						                                   '</div>'
						                                 : '<div data-payment-page-notification="danger">' . __( "No payment gateway connected.", "payment-page" ) . '</div>'
					                                 ),
    ...
    $frequency_by_value_map = payment_page_utilities_map_array(payment_page_elementor_control_pricing_frequencies(), 'value' );
    ...
    foreach ( $settings['plans'] as $key => $plan ) {
          ...
          $plan['prices'][$price_key]['frequency'] = [ 'value' => 'one-time', 'label' => 'One-time' ];
				} else if(!is_array($price['frequency'])) {
          if(!isset($frequency_by_value_map[$price['frequency']])) {
            unset( $plan['prices'][ $price_key ] );
            continue;
          }

          $plan['prices'][$price_key]['frequency'] = $frequency_by_value_map[$price['frequency']];
        }
			}
    ...
    $component_args['has_query_string_support'] = 1;
		...
		if ( isset( $settings['field_label_color'] ) ) {
			$response .= '--payment-page-element-form-label-color : ' . payment_page_form_setting_to_css_variable_color( $settings, 'field_label_color' ) . ';' . "\n";
		}

		if ( isset( $settings['field_input_color'] ) ) {
			$response .= '--payment-page-element-form-input-color : ' . payment_page_form_setting_to_css_variable_color( $settings, 'field_input_color' ) . ';' . "\n";
		}

    if ( !empty( $settings['container_max_width'] ) ) {
      $response .= '--payment-page-container-max-width : ' . _payment_page_form_setting_size_to_css( $settings['container_max_width'] ) . ';' . "\n";
    }

    if ( !empty( $settings['container_spacing_top'] ) && !empty( $settings['container_spacing_bottom'] ) ) {
      $response .= '--payment-page-container-margin : ' . _payment_page_form_setting_size_to_css( $settings['container_spacing_top'] ) . ' auto ' . _payment_page_form_setting_size_to_css( $settings['container_spacing_bottom'] ) . ' auto;' . "\n";
    }
	...
	private function _css_variable_form_pricing_plan_select( $settings ) {
		...
		if ( isset( $settings['pricing_plan_select_border_color'] ) && isset( $settings['pricing_plan_select_border_size'] ) ) {
			$response .= '--payment-page-element-form-select-border : ' . payment_page_form_setting_to_css_variable_border( $settings['pricing_plan_select_border_size'], $settings['pricing_plan_select_border_color'] ) . ';' . "\n";
		}

		if ( isset( $settings['pricing_plan_select_border_radius'] ) ) {
			$response .= '--payment-page-element-form-select-border-radius : ' . payment_page_form_setting_to_css_variable_border_radius( $settings['pricing_plan_select_border_radius'] ) . ';' . "\n";
		}


		if ( isset( $settings['pricing_plan_select_arrow_color'] ) ) {
			$response .= '--payment-page-element-form-select-arrow-color : ' . payment_page_form_setting_to_css_variable_color( $settings, 'pricing_plan_select_arrow_color' ) . ';' . "\n";
		}

		if ( isset( $settings['pricing_plan_select_background_color'] ) ) {
			$response .= '--payment-page-element-form-select-background : ' . payment_page_form_setting_to_css_variable_color( $settings, 'pricing_plan_select_background_color' ) . ';' . "\n";
		}

		if ( isset( $settings['pricing_plan_select_text_color'] ) ) {
			$response .= '--payment-page-element-form-select-color : ' . payment_page_form_setting_to_css_variable_color( $settings, 'pricing_plan_select_text_color' ) . ';' . "\n";
		}
		...
		if ( isset( $settings['pricing_plan_select_text_font_size'] ) ) {
			$response .= '--payment-page-element-form-select-font-size : ' . payment_page_form_setting_to_css_variable_font_size( $settings['pricing_plan_select_text_font_size'] ) . ';' . "\n";
		}
		...
		if ( isset( $settings['pricing_plan_select_option_background_color'] ) ) {
			$response .= '--payment-page-element-form-select-option-background : ' . payment_page_form_setting_to_css_variable_color( $settings, 'pricing_plan_select_option_background_color' ) . ';' . "\n";
		}

		if ( isset( $settings['pricing_plan_select_option_active_background_color'] ) ) {
			$response .= '--payment-page-element-form-select-option-active-background : ' . payment_page_form_setting_to_css_variable_color( $settings, 'pricing_plan_select_option_active_background_color' ) . ';' . "\n";
		}
	...
	private function _css_variable_form_payment_methods( $settings ) {
		...
		if ( isset( $settings['payment_method_item_image_height'] ) ) {
			$response .= '--payment-page-element-form-payment-method-option-image-height : ' . _payment_page_form_setting_size_to_css( $settings['payment_method_item_image_height'] ) . ';' . "\n";
		}

		if ( isset( $settings['payment_method_items_spacing'] ) ) {
			$response .= '--payment-page-element-form-payment-method-spacing : ' . _payment_page_form_setting_size_to_css( $settings['payment_method_items_spacing'] ) . ';' . "\n";
		}

		if ( isset( $settings['payment_method_section_title_style_color'] ) ) {
			$response .= '--payment-page-element-form-payment-method-title-color : ' . payment_page_form_setting_to_css_variable_color( $settings, 'payment_method_section_title_style_color' ) . ';' . "\n";
		}
		...
		if ( isset( $settings['payment_method_section_title_style_font_size'] ) ) {
			$response .= '--payment-page-element-form-payment-method-title-font-size : ' . payment_page_form_setting_to_css_variable_font_size( $settings['payment_method_section_title_style_font_size'] ) . ';' . "\n";
		}
		...
		if ( isset( $settings['global_options_title_margin'] ) ) {
			$response .= '--payment-page-element-form-payment-method-title-margin : ' . payment_page_form_setting_to_css_variable_margin( $settings['global_options_title_margin'] ) . ';' . "\n";
		}


		if ( isset( $settings['payment_method_item_inactive_padding'] ) ) {
			$response .= '--payment-page-element-form-payment-method-option-padding : ' . payment_page_form_setting_to_css_variable_padding( $settings['payment_method_item_inactive_padding'] ) . ';' . "\n";
		}

		if ( isset( $settings['payment_method_item_inactive_border_radius'] ) ) {
			$response .= '--payment-page-element-form-payment-method-option-border-radius : ' . payment_page_form_setting_to_css_variable_border_radius( $settings['payment_method_item_inactive_border_radius'] ) . ';' . "\n";
		}

		if ( isset( $settings['payment_method_item_inactive_border_color'] ) && isset( $settings['payment_method_item_inactive_border_size'] ) ) {
			$response .= '--payment-page-element-form-payment-method-option-border-top : ' . payment_page_form_setting_to_css_variable_border(
					[
						'unit' => $settings['payment_method_item_inactive_border_size']['unit'],
						'size' => ( $settings['payment_method_item_inactive_border_size']['top'] ?? $settings['payment_method_item_inactive_border_size']['size_1'] ),
					],
					payment_page_form_setting_to_css_variable_color( $settings, 'payment_method_item_inactive_border_color' )
				) . ';' . "\n";
			$response .= '--payment-page-element-form-payment-method-option-border-right : ' . payment_page_form_setting_to_css_variable_border(
					[
						'unit' => $settings['payment_method_item_inactive_border_size']['unit'],
						'size' => ( $settings['payment_method_item_inactive_border_size']['right'] ?? $settings['payment_method_item_inactive_border_size']['size_2'] ),
					],
					payment_page_form_setting_to_css_variable_color( $settings, 'payment_method_item_inactive_border_color' )
				) . ';' . "\n";
			$response .= '--payment-page-element-form-payment-method-option-border-bottom : ' . payment_page_form_setting_to_css_variable_border(
					[
						'unit' => $settings['payment_method_item_inactive_border_size']['unit'],
						'size' => ( $settings['payment_method_item_inactive_border_size']['bottom'] ?? $settings['payment_method_item_inactive_border_size']['size_3'] ),
					],
					payment_page_form_setting_to_css_variable_color( $settings, 'payment_method_item_inactive_border_color' )
				) . ';' . "\n";
			$response .= '--payment-page-element-form-payment-method-option-border-left : ' . payment_page_form_setting_to_css_variable_border(
					[
						'unit' => $settings['payment_method_item_inactive_border_size']['unit'],
						'size' => ( $settings['payment_method_item_inactive_border_size']['left'] ?? $settings['payment_method_item_inactive_border_size']['size_4'] ),
					],
					payment_page_form_setting_to_css_variable_color( $settings, 'payment_method_item_inactive_border_color' )
				) . ';' . "\n";

			$response .= '--payment-page-element-form-payment-method-option-border-color : ' . payment_page_form_setting_to_css_variable_color( $settings, 'payment_method_item_inactive_border_color' ) . ';' . "\n";
		}

		if ( isset( $settings['payment_method_item_active_padding'] ) ) {
			$response .= '--payment-page-element-form-payment-method-option-active-padding : ' . payment_page_form_setting_to_css_variable_padding( $settings['payment_method_item_active_padding'] ) . ';' . "\n";
		}

		if ( isset( $settings['payment_method_item_active_border_radius'] ) ) {
			$response .= '--payment-page-element-form-payment-method-option-active-border-radius : ' . payment_page_form_setting_to_css_variable_border_radius( $settings['payment_method_item_active_border_radius'] ) . ';' . "\n";
		}

		if ( isset( $settings['payment_method_item_active_border_color'] ) && isset( $settings['payment_method_item_active_border_size'] ) ) {
			$response .= '--payment-page-element-form-payment-method-option-active-border-top : ' . payment_page_form_setting_to_css_variable_border(
					[
						'unit' => $settings['payment_method_item_active_border_size']['unit'],
						'size' => ( $settings['payment_method_item_active_border_size']['top'] ?? $settings['payment_method_item_active_border_size']['size_1'] ),
					],
					payment_page_form_setting_to_css_variable_color( $settings, 'payment_method_item_active_border_color' )
				) . ';' . "\n";
			$response .= '--payment-page-element-form-payment-method-option-active-border-right : ' . payment_page_form_setting_to_css_variable_border(
					[
						'unit' => $settings['payment_method_item_active_border_size']['unit'],
						'size' => ( $settings['payment_method_item_active_border_size']['right'] ?? $settings['payment_method_item_active_border_size']['size_2'] ),
					],
					payment_page_form_setting_to_css_variable_color( $settings, 'payment_method_item_active_border_color' )
				) . ';' . "\n";
			$response .= '--payment-page-element-form-payment-method-option-active-border-bottom : ' . payment_page_form_setting_to_css_variable_border(
					[
						'unit' => $settings['payment_method_item_active_border_size']['unit'],
						'size' => ( $settings['payment_method_item_active_border_size']['bottom'] ?? $settings['payment_method_item_active_border_size']['size_3'] ),
					],
					payment_page_form_setting_to_css_variable_color( $settings, 'payment_method_item_active_border_color' )
				) . ';' . "\n";
			$response .= '--payment-page-element-form-payment-method-option-active-border-left : ' . payment_page_form_setting_to_css_variable_border(
					[
						'unit' => $settings['payment_method_item_active_border_size']['unit'],
						'size' => ( $settings['payment_method_item_active_border_size']['left'] ?? $settings['payment_method_item_active_border_size']['size_4'] ),
					],
					payment_page_form_setting_to_css_variable_color( $settings, 'payment_method_item_active_border_color' )
				) . ';' . "\n";
		}
	...
	private function _css_variable_form_headings( $settings ) {
		...
		if ( isset( $settings['pricing_plan_select_title_color'] ) ) {
			$response .= '--payment-page-element-form-pricing-plans-title-color : ' . payment_page_form_setting_to_css_variable_color( $settings, 'pricing_plan_select_title_color' ) . ';' . "\n";
		}
		...
		if ( isset( $settings['pricing_plan_select_title_font_size'] ) ) {
			$response .= '--payment-page-element-form-pricing-plans-title-font-size : ' . payment_page_form_setting_to_css_variable_font_size( $settings['pricing_plan_select_title_font_size'] ) . ';' . "\n";
		}
		...
		if ( isset( $settings['global_options_title_margin'] ) ) {
			$response .= '--payment-page-element-form-pricing-plans-title-margin : ' . payment_page_form_setting_to_css_variable_margin( $settings['global_options_title_margin'] ) . ';' . "\n";
		}


		if ( isset( $settings['pricing_plan_custom_amount_color'] ) ) {
			$response .= '--payment-page-element-form-custom-amount-title-color : ' . payment_page_form_setting_to_css_variable_color( $settings, 'pricing_plan_custom_amount_color' ) . ';' . "\n";
		}
		...
		if ( isset( $settings['pricing_plan_custom_amount_font_size'] ) ) {
			$response .= '--payment-page-element-form-custom-amount-title-font-size : ' . payment_page_form_setting_to_css_variable_font_size( $settings['pricing_plan_custom_amount_font_size'] ) . ';' . "\n";
		}
		...
		if ( isset( $settings['global_options_title_margin'] ) ) {
			$response .= '--payment-page-element-form-custom-amount-title-margin : ' . payment_page_form_setting_to_css_variable_margin( $settings['global_options_title_margin'] ) . ';' . "\n";
		}


		if ( isset( $settings['form_section_title_style_color'] ) ) {
			$response .= '--payment-page-element-form-payment-information-title-color : ' . payment_page_form_setting_to_css_variable_color( $settings, 'form_section_title_style_color' ) . ';' . "\n";
		}
		...
		if ( isset( $settings['form_section_title_style_font_size'] ) ) {
			$response .= '--payment-page-element-form-payment-information-title-font-size : ' . payment_page_form_setting_to_css_variable_font_size( $settings['form_section_title_style_font_size'] ) . ';' . "\n";
		}
		...
		if ( isset( $settings['global_options_title_margin'] ) ) {
			$response .= '--payment-page-element-form-payment-information-title-margin : ' . payment_page_form_setting_to_css_variable_margin( $settings['global_options_title_margin'] ) . ';' . "\n";
		}
	...
	private function _css_variables_form_label( $settings ) {
		...
		if ( isset( $settings['form_field_label_color'] ) ) {
			$response .= '--payment-page-element-form-label-color : ' . payment_page_form_setting_to_css_variable_color( $settings, 'form_field_label_color' ) . ';' . "\n";
		}
		...
		if ( isset( $settings['form_field_label_font_size'] ) ) {
			$response .= '--payment-page-element-form-label-font-size : ' . payment_page_form_setting_to_css_variable_font_size( $settings['form_field_label_font_size'] ) . ';' . "\n";
		}
		...
		if ( isset( $settings['form_field_label_active_color'] ) ) {
			$response .= '--payment-page-element-form-label-active-color : ' . payment_page_form_setting_to_css_variable_color( $settings, 'form_field_label_active_color' ) . ';' . "\n";
		}
		...
		if ( isset( $settings['form_field_label_active_font_size'] ) ) {
			$response .= '--payment-page-element-form-label-active-font-size : ' . payment_page_form_setting_to_css_variable_font_size( $settings['form_field_label_active_font_size'] ) . ';' . "\n";
		}
	...
	private function _css_variables_form_field_input( $settings ) {
		...
		if ( isset( $settings['form_field_input_color'] ) ) {
			$response .= '--payment-page-element-form-input-color : ' . payment_page_form_setting_to_css_variable_color( $settings, 'form_field_input_color' ) . ';' . "\n";
		}
		...
		if ( isset( $settings['form_field_input_font_size'] ) ) {
			$response .= '--payment-page-element-form-input-font-size : ' . payment_page_form_setting_to_css_variable_font_size( $settings['form_field_input_font_size'] ) . ';' . "\n";
		}
		...
		if ( isset( $settings['form_field_input_placeholder_color'] ) ) {
			$response .= '--payment-page-element-form-input-placeholder-color : ' . payment_page_form_setting_to_css_variable_color( $settings, 'form_field_input_placeholder_color' ) . ';' . "\n";
		}
		...
		if ( isset( $settings['form_field_input_placeholder_font_size'] ) ) {
			$response .= '--payment-page-element-form-input-placeholder-font-size : ' . payment_page_form_setting_to_css_variable_font_size( $settings['form_field_input_placeholder_font_size'] ) . ';' . "\n";
		}
	...
	private function _css_variables_form_field_container( $settings ) {
		...
		if ( isset( $settings['form_fields_border_color'] ) && isset( $settings['form_fields_border_size'] ) ) {
			$response .= '--payment-page-element-form-input-container-border : ' . payment_page_form_setting_to_css_variable_border(
					$settings['form_fields_border_size'],
					payment_page_form_setting_to_css_variable_color( $settings, 'form_fields_border_color' )
				) . ';' . "\n";

			$response .= '--payment-page-element-form-input-container-border-size: ' . _payment_page_form_setting_size_to_css( $settings['form_fields_border_size'] ) . ';' . "\n";
		}

		if ( isset( $settings['form_fields_border_radius'] ) ) {
			$response .= '--payment-page-element-form-input-container-border-radius : ' . payment_page_form_setting_to_css_variable_border_radius( $settings['form_fields_border_radius'] ) . ';' . "\n";
		}

		if ( isset( $settings['form_fields_background_color'] ) ) {
			$response .= '--payment-page-element-form-input-container-background : ' . payment_page_form_setting_to_css_variable_color( $settings, 'form_fields_background_color' ) . ';' . "\n";
		}
	...
	private function _css_variables_pricing_filters( $settings ) {
		...
		if ( isset( $settings['switcher_button_selector_border_color'] ) && isset( $settings['switcher_button_selector_border_size'] ) ) {
			$response .= '--payment-page-element-pricing-filter-container-border : ' . payment_page_form_setting_to_css_variable_border(
					$settings['switcher_button_selector_border_size'],
					payment_page_form_setting_to_css_variable_color( $settings, 'switcher_button_selector_border_color' )
				) . ';' . "\n";
		}

		if ( isset( $settings['switcher_button_selector_border_radius'] ) ) {
			$response .= '--payment-page-element-pricing-filter-container-border-radius : ' . payment_page_form_setting_to_css_variable_border_radius( $settings['switcher_button_selector_border_radius'] ) . ';' . "\n";
		}

		if ( isset( $settings['switcher_button_selector_padding'] ) ) {
			$response .= '--payment-page-element-pricing-filter-container-padding : ' . payment_page_form_setting_to_css_variable_padding( $settings['switcher_button_selector_padding'] ) . ';' . "\n";
		}
		...
		if ( isset( $settings['switcher_text_font_size'] ) ) {
			$response .= '--payment-page-element-pricing-filter-trigger-font-size : ' . payment_page_form_setting_to_css_variable_font_size( $settings['switcher_text_font_size'] ) . ';' . "\n";
		}
		...
		if ( isset( $settings['switcher_button_background_color'] ) ) {
			$response .= '--payment-page-element-pricing-filter-trigger-background : ' . payment_page_form_setting_to_css_variable_color( $settings, 'switcher_button_background_color' ) . ';' . "\n";
		}

    if ( isset( $settings['switcher_text_inactive_color'] ) ) {
      $response .= '--payment-page-element-pricing-filter-trigger-color : ' . payment_page_form_setting_to_css_variable_color( $settings, 'switcher_text_inactive_color' ) . ';' . "\n";
    } else if ( isset( $settings['switcher_text_inactive_background_color'] ) ) {
			$response .= '--payment-page-element-pricing-filter-trigger-color : ' . payment_page_form_setting_to_css_variable_color( $settings, 'switcher_text_inactive_background_color' ) . ';' . "\n";
		}

		if ( isset( $settings['switcher_button_padding'] ) ) {
			$response .= '--payment-page-element-pricing-filter-trigger-padding : ' . payment_page_form_setting_to_css_variable_padding( $settings['switcher_button_padding'] ) . ';' . "\n";
		}

		if ( isset( $settings['switcher_button_border_color'] ) && isset( $settings['switcher_button_border_size'] ) ) {
			$response .= '--payment-page-element-pricing-filter-trigger-border : ' . payment_page_form_setting_to_css_variable_border(
					$settings['switcher_button_border_size'],
					payment_page_form_setting_to_css_variable_color( $settings, 'switcher_button_border_color' )
				) . ';' . "\n";
		}

		if ( isset( $settings['switcher_button_border_radius'] ) ) {
			$response .= '--payment-page-element-pricing-filter-trigger-border-radius : ' . payment_page_form_setting_to_css_variable_border_radius( $settings['switcher_button_border_radius'] ) . ';' . "\n";
		}

		//// Active Trigger

    if ( isset( $settings['switcher_text_active_color'] ) ) {
      $response .= '--payment-page-element-pricing-filter-trigger-active-color : ' . payment_page_form_setting_to_css_variable_color( $settings, 'switcher_text_active_color' ) . ';' . "\n";
    } else if ( isset( $settings['switcher_text_active_background_color'] ) ) {
      $response .= '--payment-page-element-pricing-filter-trigger-active-color : ' . payment_page_form_setting_to_css_variable_color( $settings, 'switcher_text_active_background_color' ) . ';' . "\n";
    }

		if ( isset( $settings['switcher_button_active_background_color'] ) ) {
			$response .= '--payment-page-element-pricing-filter-trigger-active-background : ' . payment_page_form_setting_to_css_variable_color( $settings, 'switcher_button_active_background_color' ) . ';' . "\n";
		}

		if ( isset( $settings['switcher_button_active_border_color'] ) && isset( $settings['switcher_button_active_border_size'] ) ) {
			$response .= '--payment-page-element-pricing-filter-trigger-active-border : ' . payment_page_form_setting_to_css_variable_border( $settings['switcher_button_active_border_size'], $settings['switcher_button_active_border_color'] ) . ';' . "\n";
		}

		if ( isset( $settings['switcher_button_active_border_radius'] ) ) {
			$response .= '--payment-page-element-pricing-filter-trigger-active-border-radius : ' . payment_page_form_setting_to_css_variable_border_radius( $settings['switcher_button_active_border_radius'] ) . ';' . "\n";
		}

		//// Select

		if ( isset( $settings['switcher_select_background_color'] ) ) {
			$response .= '--payment-page-element-pricing-filter-select-background : ' . payment_page_form_setting_to_css_variable_color( $settings, 'switcher_select_background_color' ) . ';' . "\n";
		}

		if ( isset( $settings['switcher_select_border_color'] ) && isset( $settings['switcher_select_border_size'] ) ) {
			$response .= '--payment-page-element-pricing-filter-select-border : ' . payment_page_form_setting_to_css_variable_border(
					$settings['switcher_select_border_size'],
					payment_page_form_setting_to_css_variable_color( $settings, 'switcher_select_border_color' )
				) . ';' . "\n";
		}

		if ( isset( $settings['switcher_select_border_radius'] ) ) {
			$response .= '--payment-page-element-pricing-filter-select-border-radius : ' . payment_page_form_setting_to_css_variable_border_radius( $settings['switcher_select_border_radius'] ) . ';' . "\n";
		}

		if ( isset( $settings['switcher_select_color'] ) ) {
			$response .= '--payment-page-element-pricing-filter-select-color : ' . payment_page_form_setting_to_css_variable_color( $settings, 'switcher_select_color' ) . ';' . "\n";
		}

		if ( isset( $settings['switcher_select_font_size'] ) ) {
			$response .= '--payment-page-element-pricing-filter-select-font-size : ' . payment_page_form_setting_to_css_variable_font_size( $settings['switcher_select_font_size'] ) . ';' . "\n";
		}
		...
		if ( isset( $settings['switcher_select_arrow_color'] ) ) {
			$response .= '--payment-page-element-pricing-filter-select-arrow-color : ' . payment_page_form_setting_to_css_variable_color( $settings, 'switcher_select_arrow_color' ) . ';' . "\n";
		}
	...
	private function _css_variables_form_submit_button( $settings ) {
		...
		if ( isset( $settings['submit_button_text_color'] ) ) {
			$response .= '--payment-page-element-form-submit-color : ' . payment_page_form_setting_to_css_variable_color( $settings, 'submit_button_text_color' ) . ';' . "\n";
		}

		if ( isset( $settings['submit_button_text_font_size'] ) ) {
			$response .= '--payment-page-element-form-submit-font-size : ' . payment_page_form_setting_to_css_variable_font_size( $settings['submit_button_text_font_size'] ) . ';' . "\n";
		}
		...
		if ( isset( $settings['submit_button_padding'] ) ) {
			$response .= '--payment-page-element-form-submit-padding : ' . payment_page_form_setting_to_css_variable_padding( $settings['submit_button_padding'] ) . ';' . "\n";
		}

		if ( isset( $settings['submit_button_border_color'] ) && isset( $settings['submit_button_border_size'] ) ) {
			$response .= '--payment-page-element-form-submit-border : ' . payment_page_form_setting_to_css_variable_border( $settings['submit_button_border_size'], $settings['submit_button_border_color'] ) . ';' . "\n";
		}

		if ( isset( $settings['submit_button_border_radius'] ) ) {
			$response .= '--payment-page-element-form-submit-border-radius : ' . payment_page_form_setting_to_css_variable_border_radius( $settings['submit_button_border_radius'] ) . ';' . "\n";
		}

		if ( isset( $settings['submit_button_box_shadow'] ) && $settings['submit_button_box_shadow'] === 'yes' ) {
			$response .= '--payment-page-element-form-submit-box-shadow : ' . payment_page_form_setting_to_css_variable_box_shadow(
					$settings['submit_button_box_shadow_horizontal'], $settings['submit_button_box_shadow_vertical'], $settings['submit_button_box_shadow_blur'],
					$settings['submit_button_box_shadow_spread'], $settings['submit_button_box_shadow_color'], $settings['submit_button_box_shadow_position']
				) . ';' . "\n";
		}

		if ( isset( $settings['submit_button_margin_top'] ) ) {
			$response .= '--payment-page-element-form-submit-spacing-top : ' . payment_page_form_setting_to_css_variable_margin( $settings['submit_button_margin_top'] ) . ';' . "\n";
		}
	...
	private function _css_variable_success_page( $settings ) {
		...
		if ( isset( $settings['_payment_success_title_color'] ) ) {
			$response .= '--payment-page-success-title-color : ' . payment_page_form_setting_to_css_variable_color( $settings, '_payment_success_title_color' ) . ';' . "\n";
		}
		...
		if ( isset( $settings['_payment_success_title_font_size'] ) ) {
			$response .= '--payment-page-success-title-font-size : ' . payment_page_form_setting_to_css_variable_font_size( $settings['_payment_success_title_font_size'] ) . ';' . "\n";
		}
		...
		if ( isset( $settings['_payment_success_content_color'] ) ) {
			$response .= '--payment-page-success-message-color : ' . payment_page_form_setting_to_css_variable_color( $settings, '_payment_success_content_color' ) . ';' . "\n";
		}
		...
		if ( isset( $settings['_payment_success_content_font_size'] ) ) {
			$response .= '--payment-page-success-message-font-size : ' . payment_page_form_setting_to_css_variable_font_size( $settings['_payment_success_content_font_size'] ) . ';' . "\n";
		}
		...
		if ( isset( $settings['_payment_success_details_label_color'] ) ) {
			$response .= '--payment-page-success-payment-detail-label-color : ' . payment_page_form_setting_to_css_variable_color( $settings, '_payment_success_details_label_color' ) . ';' . "\n";
		}
		...
		if ( isset( $settings['_payment_success_details_label_font_size'] ) ) {
			$response .= '--payment-page-success-payment-detail-label-font-size : ' . payment_page_form_setting_to_css_variable_font_size( $settings['_payment_success_details_label_font_size'] ) . ';' . "\n";
		}
		...
		if ( isset( $settings['_payment_success_details_color'] ) ) {
			$response .= '--payment-page-success-payment-detail-color : ' . payment_page_form_setting_to_css_variable_color( $settings, '_payment_success_details_color' ) . ';' . "\n";
		}
		...
		if ( isset( $settings['_payment_success_details_font_size'] ) ) {
			$response .= '--payment-page-success-payment-detail-font-size : ' . payment_page_form_setting_to_css_variable_font_size( $settings['_payment_success_details_font_size'] ) . ';' . "\n";
		}
