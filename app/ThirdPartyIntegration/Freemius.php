...
class Freemius {
    ...
    public static function instance() : Freemius_Class {
        ...
        self::$_instance = fs_dynamic_init( array(
            ...
            'menu'            => array(
                ...
                'first-path' => PAYMENT_PAGE_INSTALLED_URL_PATH,
            ),
            'is_live'         => true,
        ) );
    ...
    public static function has_personal_plan() : bool {
        $site = self::instance()->get_site();
        return !empty( $site ) && in_array( intval( self::instance()->get_plan_id() ), array(11498, 12982) );
    }

    public static function has_pro_plan() : bool {
        $site = self::instance()->get_site();
        return !empty( $site ) && in_array( intval( self::instance()->get_plan_id() ), array(11499) );
    }

    public static function has_agency_plan() : bool {
        $site = self::instance()->get_site();
        return !empty( $site ) && in_array( intval( self::instance()->get_plan_id() ), array(11500, 11559) );
    }
