...
class DisabledSwitcher extends Control_Switcher {
	...
	public function content_template() {
        ...
        <label for="<?php $this->print_control_uid(); ?>" class="elementor-control-title"
               style="cursor: not-allowed !important;opacity: 0.6;">{{{ data.label }}}</label>
        <div class="elementor-control-input-wrapper">
        <# } else { #>
        <div class="elementor-control-field elementor-control-type-switcher" style="cursor: not-allowed;">
            <label for="<?php $this->print_control_uid(); ?>" class="elementor-control-title"
                   style="cursor: not-allowed !important;">{{{ data.label }}}</label>
            <div class="elementor-control-input-wrapper" style="opacity:0.6;">
                <# } #>
                <label class="elementor-switch elementor-control-unit-2">
                    <input id="<?php $this->print_control_uid(); ?>" type="checkbox" data-setting="{{ data.name }}"
                           class="elementor-switch-input" value="{{ data.return_value }}" disabled="disabled">
                    <span class="elementor-switch-label" data-on="{{ data.label_on }}" data-off="{{ data.label_off }}"
                          style="cursor: not-allowed !important;"></span>
