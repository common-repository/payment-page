...
class PaymentPage {
  ...
  public function get_form_import_templates() {
    $response = get_transient( PAYMENT_PAGE_ALIAS . '_form_templates' );

    if ( ! empty( $response ) ) {
      return $response;
    }

    $response = wp_remote_get( PAYMENT_PAGE_FORM_TEMPLATES_ENDPOINT );
    ...
    set_transient( PAYMENT_PAGE_ALIAS . '_form_templates', $response['data'], HOUR_IN_SECONDS );
